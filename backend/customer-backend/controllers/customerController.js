...
const sendLoginCredentialsEmail = async (email,password) => {
  ...
  try {
    const mailOptions = {
      from: `"RideFixr Support" <${process.env.EMAIL_USER}>`,
      to: email,
      subject,
      html
    };
...
const sendRequest = async (req, res) => {
  ...
  if (!issue || !location?.lat || !location?.lng) {
    return res.status(400).json({ message: 'Missing required fields' });
  }

  const { name, phone, id: customerId } = req.customer;
  ...
  try {
    ...
    const nearbyFixperts = await Fixpert.aggregate([
      {
        $geoNear: {
...
const getFixpertLocation = async (req, res) => {
  try {
    ...
    const request = await ServiceRequest.findOne({ customerId, status: 'accepted' });
    ...
    const { lat, lng } = fixpert.location;
